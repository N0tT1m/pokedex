...
void showPokemonDetailSheet(BuildContext context, String pokemonName) {
  showModalBottomSheet(
    ...
    builder: (ctx) =>
        _PokemonDetailSheetContent(apiName: _resolveApiName(pokemonName)),
  );
}
...
class _PokemonDetailBodyState extends State<_PokemonDetailBody> {
  ...
  Future<void> _load() async {
    try {
      ...
      if (mounted) {
        setState(() {
          _error = 'Could not load Pokemon data';
          _isLoading = false;
        });
      }
    }
  ...
  Future<void> _loadForName(String name) async {
    ...
    await Future.wait([
      ...
      Requests.get('${PokeApiService.baseUrl}/pokemon/$name/held-items')
          .then((r) {
        if (r.statusCode == 200) {
          heldItems =
              List<Map<String, dynamic>>.from(r.json()['held_items'] ?? []);
        }
      }).catchError((_) {}),
      Requests.get('${PokeApiService.baseUrl}/pokemon/$name/game-locations')
          .then((r) {
        if (r.statusCode == 200) {
          gameLocations =
              List<Map<String, dynamic>>.from(r.json()['locations'] ?? []);
        }
      }).catchError((_) {}),
      Requests.get('${PokeApiService.baseUrl}/pokemon/$name/classification')
          .then((r) {
        if (r.statusCode == 200) {
          classification = Map<String, dynamic>.from(r.json());
        }
      }).catchError((_) {}),
      Requests.get('${PokeApiService.baseUrl}/pokemon/$name/go').then((r) {
        if (r.statusCode == 200) goStats = Map<String, dynamic>.from(r.json());
      }).catchError((_) {}),
      Requests.get('${PokeApiService.baseUrl}/pokemon/$name/contest-stats')
          .then((r) {
        if (r.statusCode == 200) {
          contestStats =
              List<Map<String, dynamic>>.from(r.json()['contest_stats'] ?? []);
        }
      }).catchError((_) {}),
    ]);
    ...
    final abilityDetails = await Future.wait(rawAbilities.map((a) async {
      ...
      try {
        final r =
            await Requests.get('${PokeApiService.baseUrl}/ability/$apiName');
        if (r.statusCode == 200) {
  ...
  Widget build(BuildContext context) {
...

    {
      ...
      final sprite = data['sprites']?['other']?['official-artwork']
              ?['front_default'] ??
          data['sprites']?['front_default'] ??
          '';
      final bst = (data['stats'] as List)
          .fold<int>(0, (sum, s) => sum + (s['base_stat'] as int));
                          ...
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: Border.all(
                                    color: _methodColor(method), width: 1),
                    ...
                    Wrap(
                      ...
                      children: [
                        if (_classification!['generation_introduced'] != null)
                          _infoBadge(
                              'Gen ${_classification!['generation_introduced']}',
                              Colors.blue),
                        if (_classification!['color'] != null)
                          _infoBadge(_capitalize(_classification!['color']),
                              Colors.blueGrey),
                        if (_classification!['shape'] != null)
                          _infoBadge(_capitalize(_classification!['shape']),
                              Colors.teal),
                        if (_classification!['habitat'] != null)
                          _infoBadge(_capitalize(_classification!['habitat']),
                              Colors.green),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(
                            child: _goStatBox(
                                'Max CP', '${_goStats!['max_cp'] ?? '-'}')),
                        Expanded(
                            child: _goStatBox('Attack',
                                '${_goStats!['base_attack'] ?? '-'}')),
                        Expanded(
                            child: _goStatBox('Defense',
                                '${_goStats!['base_defense'] ?? '-'}')),
                        Expanded(
                            child: _goStatBox('Stamina',
                                '${_goStats!['base_stamina'] ?? '-'}')),
                      ],
                    ),
                    const SizedBox(height: 8),
                    Row(
                      children: [
                        if (_goStats!['buddy_distance_km'] != null)
                          _infoBadge(
                              '${_goStats!['buddy_distance_km']} km buddy',
                              Colors.orange),
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Contest Stats',
                        style: TextStyle(
                            fontWeight: FontWeight.bold, fontSize: 15)),
                    const SizedBox(height: 8),
                    ..._contestStats.map((s) {
                      final type =
                          _capitalize(s['contest_type'] as String? ?? '');
        ...
        children: weaknesses
            .map((t) => Container(
                  ...
                  decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(8)),
                  child: Text('${t['type_name']} ${t['multiplier']}x',
                      style: TextStyle(
                          fontSize: 11,
          ...
          children: [
            ...
            Expanded(
                child: _PokemonDetailBody(
                    apiName: apiName, scrollController: scrollController)),
          ],
        );
      },
    );
