...
class CatchCalculatorScreen extends StatefulWidget {
  const CatchCalculatorScreen({super.key});
...
class _CatchCalculatorScreenState extends State<CatchCalculatorScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: const Text('Catch Calculator'), backgroundColor: Colors.red),
      body: _isLoading
                        ...
                        children: [
                          ...
                          if (_selectedPokemon != null)
                            Padding(
                              ...
                              child: Text(
                                  'Base catch rate: $_baseCatchRate / 255',
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            decoration: const InputDecoration(
                                labelText: 'Ball Type',
                                border: OutlineInputBorder()),
                            items: CatchCalculatorService.allBalls
                                .map((b) =>
                          ...
                          const SizedBox(height: 12),
                          DropdownButtonFormField<String>(
                            initialValue: _statusCondition,
                            isExpanded: true,
                            decoration: const InputDecoration(
                                labelText: 'Status',
                                border: OutlineInputBorder()),
                            items: CatchCalculatorService.allStatuses
                                .map((s) =>
                                    DropdownMenuItem(value: s, child: Text(s)))
                                .toList(),
                            onChanged: (v) =>
                                setState(() => _statusCondition = v!),
                          ),
                          const SizedBox(height: 8),
                          Wrap(
                            spacing: 8,
                            children: [
                              FilterChip(
                                  ...
                                  onSelected: (v) =>
                                      setState(() => _isNight = v)),
                              FilterChip(
                                  label: const Text('Cave'),
                                  selected: _isInCave,
                                  onSelected: (v) =>
                                      setState(() => _isInCave = v)),
                              FilterChip(
                                  label: const Text('Water'),
                                  selected: _isInWater,
                                  onSelected: (v) =>
                                      setState(() => _isInWater = v)),
                            ],
                          ),
                        ],
                      ),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const Text('catch probability',
                style: TextStyle(color: Colors.grey)),
            const Divider(),
            _resultRow('Per-shake probability',
                '${(r['shakeProbability'] as double).toStringAsFixed(1)}%'),
