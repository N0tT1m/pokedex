...
class BreedingChainScreen extends StatefulWidget {
  const BreedingChainScreen({super.key});
...
class _BreedingChainScreenState extends State<BreedingChainScreen>
  ...
  Future<void> _loadMoves(String pokemonName) async {
    try {
      final moves =
          await PokeApiService.getPokemonMoves(pokemonName.toLowerCase());
      if (!mounted) return;
      final methodOrder = ['egg', 'level-up', 'tm', 'tutor'];
      final normalized = moves
          .map((m) {
            final raw = (m['learn_method'] as String? ?? '').toLowerCase();
            final apiName = m['name'] as String? ?? '';
            final displayName = apiName
                .split('-')
                .map((w) =>
                    w.isNotEmpty ? w[0].toUpperCase() + w.substring(1) : w)
                .join(' ');
            final method = raw == 'level-up'
                ? 'level-up'
                : raw == 'tm'
                    ? 'tm'
                    : raw == 'egg'
                        ...
                        : raw == 'tutor'
                            ...
                            : null;
            return method != null
                ? {'apiName': apiName, 'name': displayName, 'method': method}
                : null;
          })
  ...
  Future<void> _findChain() async {
    ...
    try {
        ...
        if (response.statusCode == 200) {
          ...
          if (parents.isNotEmpty) {
            ...
            for (final p in parents) {
              ...
              try {
                final pokeData =
                    await PokeApiService.getPokemon(parentName.toLowerCase());
      ...
      final targetSpecies =
          await PokeApiService.getPokemonSpecies(_targetPokemon!);
      final targetEggGroups = (targetSpecies['egg_groups'] as List)
          .map((g) => g['name'] as String)
          .toList();

      final moveResponse =
          await Requests.get('${PokeApiService.baseUrl}/move/$moveApiName');
      if (!mounted) return;
      if (moveResponse.statusCode != 200) {
        setState(() {
          _errorMessage = 'Could not load move data';
          _searching = false;
        });
        return;
      }
      ...
      for (var name in learnedBy) {
        try {
          ...
          final eggGroups = (species['egg_groups'] as List)
              ...
              .toList();
          final shared =
              eggGroups.where((g) => targetEggGroups.contains(g)).toList();
          if (shared.isNotEmpty) {
            ...
            for (var move in moves) {
              if (move['name'] == _targetMove) {
                ...
                if (methodLower.contains('level') ||
                    methodLower.contains('tm') ||
                    methodLower.contains('machine') ||
                    methodLower.contains('tutor')) {
                  learnMethod = method;
                  break;
                }
              }
            }
            if (learnMethod != 'unknown' &&
                !learnMethod.toLowerCase().contains('egg')) {
              int? pokeId;
              try {
      ...
      setState(() {
        ...
        if (directParents.isEmpty) {
          _errorMessage =
              'No direct breeding parents found. The move may require a multi-step chain.';
        }
      });
    } catch (e) {
      if (!mounted) return;
      ...
      child: Column(
        ...
        children: [
          ...
          if (_targetMove != null)
            ElevatedButton(
              ...
              child: _searching
                  ...
                  : Text('Find Parents for ${_fmt(_targetMove!)}',
                      style:
          ...
          if (_chainResults.isNotEmpty) ...[
            ...
            const SizedBox(height: 4),
            Text(
                'These Pokémon can pass ${_fmt(_targetMove!)} to ${_fmt(_targetPokemon!)} as an egg move.',
            ...
            ..._chainResults.map((parent) {
              ...
              return Card(
                child: ListTile(
                  ...
                  title: Text(
                      parentId != null
                          ? '#$parentId ${_fmt(parent['name'])}'
                          : _fmt(parent['name']),
                      style: const TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: Text('Learns via ${_fmt(parent['learnMethod'])}\n'
              ...
              child: Padding(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      _execStep(
                          'Gen 2–8',
                          'The FATHER must know ${_fmt(_targetMove!)}. Breed a male parent from the list above '
                              'with a female ${_fmt(_targetPokemon!)} (or a Ditto). The baby will hatch with the egg move.'),
                      _execStep(
                          'Gen 9 (SV)',
                          'Either parent can pass the move. You can also use a Mirror Herb: '
                      ...
                      _execStep(
                          'Multi-step',
                          'If no parent above is the right gender or has other moves you need, breed the move '
                ...
                child: Column(
                    ...
                    children: [
                      Row(children: [
                        ...
                        const SizedBox(width: 6),
                        Text('Passing IVs, Nature & Ability',
                            style: TextStyle(
                                fontWeight: FontWeight.bold,
                                fontSize: 14,
                      ...
                      _execStep(
                          'IVs',
                          'Give one parent a Destiny Knot (Gen 6+) to pass 5 of the 12 combined IVs. '
                      ...
                      _execStep(
                          'Power Items',
                          'Power Bracer (Atk), Belt (Def), Lens (SpA), Band (SpD), Anklet (Spe), Weight (HP) — '
                      ...
                      _execStep(
                          'Nature',
                          'Give the parent with the desired Nature an Everstone — the baby '
                      ...
                      _execStep(
                          'Ability',
                          'Females pass Hidden Ability at 60% chance. Males/genderless '
                      ...
                      _execStep(
                          'Ball',
                          'Offspring inherits the mother\'s Poké Ball (or the non-Ditto parent\'s). '
                              'Same-species pairs: 50/50 which parent\'s ball is used. Master Ball and Cherish Ball cannot be inherited.'),
                      const Divider(height: 16),
                      Text(
                          'Optimal Setup: Destiny Knot on the high-IV parent, '
            ...
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    _svStep(1,
                        'Buy a Mirror Herb from Delibird Presents (unlocked after 3+ Gym Badges). Costs \u20BD30,000.'),
                    _svStep(2,
                        'You need two Pokémon of the SAME SPECIES. Pokémon A already knows the egg move. Pokémon B is the one you want to teach.'),
                    _svStep(3,
                        'Make sure Pokémon B has an empty move slot. If all 4 slots are full, go to the Move Relearner and forget one.'),
                    _svStep(4, 'Give Pokémon B the Mirror Herb item.'),
                    _svStep(5, 'Put both in your party and start a Picnic.'),
                    _svStep(6,
                        'End the Picnic after a few seconds. Pokémon B will now know the egg move. The Mirror Herb is consumed.'),
                  ]),
            ),
          ),
            ...
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    _infoBlock(
                        'Lv.1 Recipes (Common Ingredients)',
                    ...
                    _infoBlock(
                        'Lv.3 Recipes (Herba Mystica Required)',
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _infoCard(
              'Generation Differences Summary',
              Colors.grey,
              Icons.history,
