...
class ShinyCalculatorScreen extends StatefulWidget {
  const ShinyCalculatorScreen({super.key});
...
class _ShinyCalculatorScreenState extends State<ShinyCalculatorScreen> {
  ...
  int get _generation {
    if (_selectedGame.contains('Scarlet') ||
        _selectedGame.contains('Legends')) {
      return 9;
    }
    if (_selectedGame.contains('Sword') ||
        _selectedGame.contains('Brilliant')) {
      return 8;
    }
    if (_selectedGame.contains('Let\'s Go')) return 7;
    if (_selectedGame.contains('Sun') || _selectedGame.contains('Ultra')) {
      return 7;
    }
    if (_selectedGame.contains('X/Y') || _selectedGame.contains('Omega')) {
      return 6;
    }
    if (_selectedGame.contains('Black')) return 5;
    if (_selectedGame.contains('Diamond') || _selectedGame.contains('Heart')) {
      return 4;
    }
    if (_selectedGame.contains('Ruby') || _selectedGame.contains('Fire')) {
      return 3;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: const Text('Shiny Calculator'), backgroundColor: Colors.red),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: ShinyCalculatorService.methodsByGame.keys
                    ...
                    SwitchListTile(
                      ...
                      onChanged: _generation >= 5
                          ? (v) => setState(() => _hasShinyCharm = v)
                          : null,
                    ),
                  ],
                ),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Slider(
                        ...
                        min: 0,
                        max: _chainMax.toDouble(),
                        divisions: _chainMax,
                        label: '$_chainLength',
                        onChanged: (v) =>
                            setState(() => _chainLength = v.toInt()),
                      ),
                      Center(
                          child: Text('$_chainLength',
            ...
            if (_selectedMethod == 'KO Method (Sw/Sh)')
              Card(
                child: Padding(
                  padding: const EdgeInsets.all(12),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Pokemon KO\'d',
                          style: TextStyle(fontWeight: FontWeight.bold)),
                      Slider(
                        value: _koCount.toDouble(),
                        min: 0,
                        max: 500,
                        divisions: 50,
                        label: '$_koCount',
              ...
              Card(
                child: Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('DexNav Search Level',
                          style: TextStyle(fontWeight: FontWeight.bold)),
                      Slider(
                        value: _dexNavLevel.toDouble(),
                        min: 0,
                        max: 999,
                        divisions: 100,
                        label: '$_dexNavLevel',
                        onChanged: (v) =>
                            setState(() => _dexNavLevel = v.toInt()),
                      ),
                      Center(
                          child: Text('$_dexNavLevel',
                              style: const TextStyle(
                                  fontSize: 18, fontWeight: FontWeight.bold))),
                    ],
                  ),
            ...
            if (_selectedMethod == 'Sandwich Power (SV)')
              Card(
                child: Padding(
                  padding: const EdgeInsets.all(12),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Sparkling Power Level',
                          style: TextStyle(fontWeight: FontWeight.bold)),
                      Slider(
                        value: _sandwichPower.toDouble(),
                        min: 0,
                        max: 3,
                        divisions: 3,
                        label: 'Lv $_sandwichPower',
                        onChanged: (v) =>
                            setState(() => _sandwichPower = v.toInt()),
                      ),
                      Center(
                          child: Text('Level $_sandwichPower',
                              style: const TextStyle(
                                  fontSize: 18, fontWeight: FontWeight.bold))),
                    ],
                  ),
  ...
  bool get _needsChainInput {
    return ['Chain Fishing', 'PokeRadar', 'SOS Chaining', 'Catch Combo (LGPE)']
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Text(
                '${(r['probability'] as double).toStringAsFixed(4)}% per encounter',
