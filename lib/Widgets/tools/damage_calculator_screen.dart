...
class _DamageCalculatorScreenState extends State<DamageCalculatorScreen> {
  ...
  void _calculate() {
    ...
    if (_moveCategory == 'physical') {
      atkStat = IVCalculatorService.calculateSingleStat(
          baseStat: atkStats['Attack']!,
          iv: 31,
          ev: 252,
          level: atkLevel,
          natureModifier: atkNatureMods['Attack']!,
          isHP: false);
      defStat = IVCalculatorService.calculateSingleStat(
          baseStat: defStats['Defense']!,
          iv: 31,
          ev: 252,
          level: defLevel,
          natureModifier: defNatureMods['Defense']!,
          isHP: false);
    } else {
      atkStat = IVCalculatorService.calculateSingleStat(
          baseStat: atkStats['Sp. Atk']!,
          iv: 31,
          ev: 252,
          level: atkLevel,
          natureModifier: atkNatureMods['Sp. Atk']!,
          isHP: false);
      defStat = IVCalculatorService.calculateSingleStat(
          baseStat: defStats['Sp. Def']!,
          iv: 31,
          ev: 252,
          level: defLevel,
          natureModifier: defNatureMods['Sp. Def']!,
          isHP: false);
    }

    final defHP = IVCalculatorService.calculateSingleStat(
        baseStat: defStats['HP']!,
        iv: 31,
        ev: 252,
        level: defLevel,
        natureModifier: 1.0,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: const Text('Damage Calculator'), backgroundColor: Colors.red),
      body: _isLoading
              ...
              child: Column(
                ...
                children: [
                  _buildPokemonSelector(
                      'Attacker', _attackerName, _attackerData, true),
                  if (_attackerData != null)
                    _buildLevelNature(_atkLevelCtrl, _atkNature,
                        (v) => setState(() => _atkNature = v)),
                  const SizedBox(height: 16),
                  _buildPokemonSelector(
                      'Defender', _defenderName, _defenderData, false),
                  if (_defenderData != null)
                    _buildLevelNature(_defLevelCtrl, _defNature,
                        (v) => setState(() => _defNature = v)),
                  const SizedBox(height: 16),
                  _buildMoveInput(),
                  const SizedBox(height: 16),
                  _buildOptions(),
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                        ...
                        padding: const EdgeInsets.all(16)),
                    child: const Text('Calculate Damage',
  ...
  Widget _buildPokemonSelector(String label, String? selected,
      Map<String, dynamic>? data, bool isAttacker) {
    return Card(
      child: Padding(
  ...
  Widget _buildLevelNature(TextEditingController levelCtrl, String nature,
      Function(String) onNatureChanged) {
    return Padding(
        ...
        children: [
          ...
          Expanded(
            ...
            child: DropdownButtonFormField<String>(
              ...
              isExpanded: true,
              decoration: const InputDecoration(
                  labelText: 'Nature', border: OutlineInputBorder()),
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    keyboardType: TextInputType.number,
                    decoration: const InputDecoration(
                        labelText: 'Power', border: OutlineInputBorder()),
            ...
            DropdownButtonFormField<String>(
              ...
              items: TypeEffectivenessService.allTypes
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          children: [
            SwitchListTile(
                title: const Text('Critical Hit'),
                value: _isCritical,
                onChanged: (v) => setState(() => _isCritical = v)),
            SwitchListTile(
                title: const Text('Burned (Attacker)'),
                value: _isBurned,
                onChanged: (v) => setState(() => _isBurned = v)),
            Row(
              children: [
                const Text('  Weather: '),
                ChoiceChip(
                    label: const Text('None'),
                    selected: _weatherMod == 1.0,
                    onSelected: (_) => setState(() => _weatherMod = 1.0)),
                const SizedBox(width: 4),
                ChoiceChip(
                    label: const Text('Boost'),
                    selected: _weatherMod == 1.5,
                    onSelected: (_) => setState(() => _weatherMod = 1.5)),
                const SizedBox(width: 4),
                ChoiceChip(
                    label: const Text('Weaken'),
                    selected: _weatherMod == 0.5,
                    onSelected: (_) => setState(() => _weatherMod = 0.5)),
              ],
            ),
          ],
        ),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 8),
            Text('${r['min']} - ${r['max']} damage',
                style: const TextStyle(fontSize: 18)),
            Text(
                '${(r['minPercent'] as double).toStringAsFixed(1)}% - ${(r['maxPercent'] as double).toStringAsFixed(1)}%',
            ...
            if (r['stab'] == true)
              const Text('STAB applied', style: TextStyle(color: Colors.blue)),
          ],
        ),
      ),
    );
