...
class _NuzlockeTrackerScreenState extends State<NuzlockeTrackerScreen> {
            ...
            children: [
              TextField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Run Name', border: OutlineInputBorder())),
          ...
          content: Column(
            ...
            children: [
              ...
              TextField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Pokemon', border: OutlineInputBorder())),
              const SizedBox(height: 12),
              TextField(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
            _selectedRun != null ? _selectedRun!.name : 'Nuzlocke Tracker'),
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          child: ListTile(
            ...
            subtitle: Text(
                '${run.gameName} | ${run.aliveCount} alive | ${run.deadCount} dead'),
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                if (run.isActive)
  ...
  Widget _buildRunDetail() {
    ...
    final party =
        run.encounters.where((e) => e.status == 'alive' && e.inParty).toList();
    final alive =
        run.encounters.where((e) => e.status == 'alive' && !e.inParty).toList();
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (alive.isNotEmpty)
            _buildSection('Alive (not in party)', alive, run, Colors.green),
          if (boxed.isNotEmpty)
            _buildSection('Boxed', boxed, run, Colors.orange),
  ...
  Widget _buildSection(String title, List<NuzlockeEncounter> encounters,
      NuzlockeRun run, Color color) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const SizedBox(height: 12),
        ...
        ...encounters.map((e) {
          ...
          return Card(
            child: ListTile(
              title: Text(e.nickname ?? e.pokemonName,
                  style: const TextStyle(fontWeight: FontWeight.bold)),
              ...
              trailing: Row(
                ...
                children: [
                  if (e.status == 'alive')
                    IconButton(
                      icon: Icon(e.inParty ? Icons.star : Icons.star_border,
                          color: e.inParty ? Colors.amber : null),
