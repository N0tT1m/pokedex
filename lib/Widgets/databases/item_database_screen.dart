...
class _ItemDatabaseScreenState extends State<ItemDatabaseScreen> {
  ...
  Future<void> _loadItems() async {
    try {
      final response =
          await Requests.get('${PokeApiService.baseUrl}/item?limit=2000');
  ...
  Future<void> _loadItemDetail(String name) async {
    ...
    try {
      final response =
          await Requests.get('${PokeApiService.baseUrl}/item/$name');
      if (response.statusCode == 200) {
        setState(() {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_selectedItem != null
            ? _formatName(_selectedItem!['name'])
            : 'Item Database'),
              ...
              child: Column(
                children: [
                  ...
                  if (category.isNotEmpty)
                    Text(_formatName(category),
                        style: const TextStyle(color: Colors.grey)),
                  if (cost > 0)
                    Text('Cost: $cost', style: const TextStyle(fontSize: 14)),
                  const SizedBox(height: 12),
                  if (shortEffect.isNotEmpty)
                    Text(shortEffect, textAlign: TextAlign.center),
                ],
              ),
            ),
          ),
