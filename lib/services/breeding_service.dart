...
class BreedingService {
  ...
  static Future<Map<String, dynamic>> checkCompatibility(
      String pokemon1, String pokemon2) async {
    try {
      final species1 =
          await PokeApiService.getPokemonSpecies(pokemon1.toLowerCase());
      final species2 =
          await PokeApiService.getPokemonSpecies(pokemon2.toLowerCase());
      ...
      bool dittoInvolved =
          eggGroups1.contains('ditto') || eggGroups2.contains('ditto');
      bool undiscovered =
          eggGroups1.contains('no-eggs') || eggGroups2.contains('no-eggs');
      ...
      if (undiscovered && !dittoInvolved) {
        return {
          ...
          'compatible': false,
          'reason':
              'One or both Pokémon are in the Undiscovered egg group and cannot breed.',
        };
      }
      ...
      final sharedGroups =
          eggGroups1.where((g) => eggGroups2.contains(g)).toList();
      ...
      if (sharedGroups.isNotEmpty) {
        return {
          ...
          'compatible': true,
          'reason':
              'Shared egg group(s): ${sharedGroups.map(_formatEggGroup).join(", ")}',
      ...
      return {
        ...
        'compatible': false,
        'reason':
            'No shared egg groups. These Pokémon cannot breed with each other.',
      };
    } catch (e) {
      return {
        'compatible': false,
  ...
  static Map<String, dynamic> _extractSpeciesInfo(
      Map<String, dynamic> species) {
    final genderRate =
        species['gender_rate']; // -1=genderless, 0=always M, 8=always F
    String genderText;
    if (genderRate == -1) {
  ...
  static Future<List<Map<String, dynamic>>> getEggMoves(
