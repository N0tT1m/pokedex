...
class CoverageAnalyzerService {
  ...
  static Map<String, int> analyzeDefensiveWeaknesses(
      List<List<String>> teamTypes) {
    final weaknesses = <String, int>{};
    for (var atkType in TypeEffectivenessService.allTypes) {
      int weakCount = 0;
      for (var memberTypes in teamTypes) {
        double eff = TypeEffectivenessService.getCombinedEffectiveness(
            atkType, memberTypes);
  ...
  static Map<String, int> analyzeDefensiveResistances(
      List<List<String>> teamTypes) {
  ...
  static Map<String, bool> analyzeOffensiveCoverage(
      List<List<String>> teamMoveTypes) {
    ...
    for (var defType in TypeEffectivenessService.allTypes) {
      ...
      for (var memberMoves in teamMoveTypes) {
        for (var moveType in memberMoves) {
          if (TypeEffectivenessService.getEffectiveness(moveType, defType) >
              1.0) {
  ...
  static List<String> getUncoveredTypes(List<List<String>> teamMoveTypes) {
    ...
    return coverage.entries.where((e) => !e.value).map((e) => e.key).toList();
  }

  /// Get shared weaknesses (types multiple team members are weak to)
  static List<MapEntry<String, int>> getSharedWeaknesses(
      List<List<String>> teamTypes) {
