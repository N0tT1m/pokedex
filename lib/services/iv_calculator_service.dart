class IVCalculatorService {
  ...
  static Map<String, int> calculateIVRange({
    ...
    // The loop ascends 0->31, so the first match is the minimum IV and the
    // last is the maximum. Use nullable sentinels so an IV of 0 is a valid
    // match rather than being mistaken for "nothing found yet".
    int? minIV;
    int? maxIV;
    ...
    // If we found valid IVs, return the range (including a genuine 0).
  ...
  static int calculateSingleStat({
    ...
    int finalStat;
    if (isHP) {
      // HP formula: ((2 * Base + IV + floor(EV / 4)) * Level / 100) + Level + 10
      finalStat = (((2 * baseStat + iv + (ev ~/ 4)) * level) / 100).floor() +
          level +
          10;
    } else {
      // Other stats: (((2 * Base + IV + floor(EV / 4)) * Level / 100) + 5) * Nature
      finalStat =
