...
class CatchCalculatorService {
  ...
  static double _getBallModifier(
    String ball, {
    bool isNight = false,
    bool isInWater = false,
    bool isInCave = false,
    ...
    switch (ball) {
      ...
      case 'Dive Ball':
        return isInWater ? 3.5 : 1.0;
      // (41 - level)/10 for low levels, floored at 1.0 (so L30 → 1.1, L31+ → 1.0).
      case 'Nest Ball':
        return max(1.0, (41 - level) / 10.0);
      case 'Repeat Ball':
        return 3.5; // Assumes already caught
      // Modifier scales with turns *passed*, so turn 1 (0 elapsed) is 1.0×.
      case 'Timer Ball':
        return min(4.0, 1.0 + max(0, turnCount - 1) * 1229 / 4096);
      case 'Luxury Ball':
        return 1.0;
      case 'Premier Ball':
        return 1.0;
      case 'Dusk Ball':
        return (isNight || isInCave) ? 3.0 : 1.0;
      case 'Heal Ball':
        return 1.0;
      case 'Quick Ball':
      ...
      case 'Lure Ball':
        return isInWater ? 4.0 : 1.0;
      case 'Moon Ball':
        return 4.0; // For Moon Stone evolutions
      case 'Heavy Ball':
        return 1.0; // Flat modifier, simplified
      case 'Love Ball':
        return 8.0; // Assumes opposite gender
      case 'Friend Ball':
        return 1.0;
      case 'Fast Ball':
        return 4.0; // For Pokemon with 100+ Speed
      case 'Dream Ball':
        return 4.0; // For sleeping Pokemon
      case 'Beast Ball':
        return 0.1; // Very low for non-UBs
      default:
        return 1.0;
    }
  }
